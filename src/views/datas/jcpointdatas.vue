<template>
    <div style="display:flex;justify-content: space-between;align-items: center;">
       ...
       <template #content>
            <span class="text-large font-600 mr-3"> 监测点数据</span>
       </template>
     </el-page-header>
  ...
  <el-select v-model="type" class="m-2" placeholder="Select" style="width:100px;margin-right:20px">
    <el-option
    v-for="item in types"
      :key="item.value"
      :label="item.label"
  ...
  let name = year + "" + month + "" + day;
    ...
    const id=router.currentRoute.value.query.id
     ...
     startLoading()
     axios.get('https://e7luck.cn/hhycc/visual/exceljcdata?pjid='+id+'&ctype='+ctype.value+'&type='+type.value).then(res=>{
        tableData.length=0
