<template>
  ...
  <template #content>
    <span class="text-large font-600 mr-3"> 防控区边界管理 </span>
  </template>
</el-page-header>
<el-select placeholder="请选择防控区" style="margin-left:30%;width: 15%;" v-model="area" clearable>
  <el-option
    v-for="item in areas"
    :key="item.value"
    :label="item.label"
...
const drawold=(aname)=>{
 ...
 reset();
 oldaname=aname
  axios.get('https://e7luck.cn/hhycc/user/fkguijis?pjid='+id+'&ctype='+2+'&aid='+aname+'&pid='+0).then(res=>{
    console.log(res);
...
// 样区点
axios.get('https://e7luck.cn/hhycc/user/mapfkdata?pid='+id+'&ctype='+2+'&aid='+aname).then(res=>{
...
const save=()=>{
  ...
  if(area.value==''){
    ElMessage.error('请先选择所要管理边界的防控区')
    reset()
  }else if(polnglat==undefined||polnglat==''){
...
// 计算面积接口
    ...
    //   console.log(res)
    // })
    console.log(polnglats)
    // 存储边界
    axios.post('https://e7luck.cn/hhycc/user/udpjarea?id='+area.value+'&lanlats='+polnglats+'&carea='+realVal).then(res=>{
      console.log(res)
      reset()
    drawold(oldaname);
      ElMessage({
